...
def test_normalizes_endpoint_and_default_port():
    endpoint = endpoint_from_url(" HTTPS://Model.Internal./health ")
    assert endpoint.key == ("https", "model.internal", 443)

def test_rejects_userinfo():
    with pytest.raises(EndpointPolicyError) as exc:
        endpoint_from_url("https://user:pass@example.com/")
    assert exc.value.code == "userinfo_present"

def test_rejects_non_http_scheme():
    with pytest.raises(EndpointPolicyError) as exc:
        endpoint_from_url("file:///etc/passwd")
